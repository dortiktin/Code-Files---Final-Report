...
class scan_area():
    ...
    def update_sight_distances_points(self):  # after each movement of husky
        self.sight_distances, self.sight_points = [], []
        for i in range(len(self.sight_garbages)):
            if(self.sight_garbages[i]._name in self.collected_names):
                ...
                point_global.point.x, point_global.point.y, point_global.point.z = self.sight_garbages[i].garbage_point[0], self.sight_garbages[i].garbage_point[1], 0.0
                ...
                xy = np.array([point.point.x, point.point.y])
                self.sight_points.append(xy)
                self.sight_distances.append(np.linalg.norm(xy))
    ...
    def go_scan(self, goal_pose):  # go to goal_pose
            ...
            self.update_sight_distances_points()        
    ...
    def main(self):
            ...
            if arrive_start_point:
                while 1:  # until break
                    ...
                    for g in range(len(self.sight_points)):
                        ...
                        print(u"\u001b[34mcurrent sight distances: " + str(np.round(self.sight_distances, 2)))              
            ...
            rospy.sleep(0.5)  # to ensure stopping the publishing before unregister 
